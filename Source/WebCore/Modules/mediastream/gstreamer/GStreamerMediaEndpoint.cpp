...
void GStreamerMediaEndpoint::doSetRemoteDescription(const RTCSessionDescription& description)
{
    ...
    setDescription(&description, DescriptionType::Remote, [this](const auto& message) {
        ...
        // To comply with https://w3c.github.io/webrtc-pc/#set-remote-description, we need to have the set of transceivers
        // created when setting the remote offer. However, as of GStreamer 1.24.4, the transceivers in webrtcbin are only
        // created once an answer is created, which only depends on having the remote offer set beforehand.
        // To force having the transceivers created, we trigger answer creation early, but we do not use the result or expose
        // the answer to JS yet. It's ok to do this because the only pre-requisite to creating an answer is having the
        // remote offer set beforehand and we have it at this point. This will just trigger the creation of transceivers
        // internally in webrtcbin and is fine to be called again later.
        // TODO: make this version-dependent once this is fixed in a released version of GStreamer.
        ...
        processSDPMessage(&message, [this](unsigned /*mediaIdx*/, const char* mid, const auto* media) {
            ...
            // setTransceiverCodecPreferences(*media, mediaIdx);
        });
...
void GStreamerMediaEndpoint::collectTransceivers()
{
    ...
    for (unsigned i = 0; i < transceivers->len; i++) {
        ...
        GST_TRACE_OBJECT(m_webrtcBin.get(), "transceiver %u: %" GST_PTR_FORMAT, i, current.get());
        ...
        // As of GStreamer 1.24.4, webrtcbin's transceivers only have the "mid" property set after the signaling state is STABLE.
        // However, we need to use the transceiver mid property when signaling is still HAVE_REMOTE_OFFER. For that, we set
        // the mid from the SDP directly as an alternative for using it in this early stage.
        // TODO: make this version-dependent if it changes in a future version of GStreamer.
