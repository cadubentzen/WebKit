 ...
 *   - HTML-based select dropdown (popup menu rendered as DOM overlay)
...
/* Saved state for option menu action handling */
static WebKitOptionMenu* savedOptionMenu = nullptr;
...
/* --- HTML Option Menu (Select Dropdown) --- */

/*
 * Option menu CSS — dark theme, consistent with context menu styling.
 * All selectors prefixed with __wpe_ to avoid conflicts with page CSS.
 */
static const char* optionMenuCSS = R"CSS(
#__wpe_opt_overlay {
    position: fixed;
...
.__wpe_opt_item:hover {
    background: #0060df;
    color: #ffffff;
}
.__wpe_opt_item:disabled,
.__wpe_opt_disabled {
    color: #707070;
    cursor: default;
    pointer-events: none;
}
.__wpe_opt_selected:before {
    content: '\\2713';
    position: absolute;
    left: 10px;
}
...
/* Dismiss the HTML option menu overlay and close the WebKitOptionMenu */
static void dismissOptionMenu(WebKitWebView* webView)
{
    webkit_web_view_evaluate_javascript(webView,
        "document.getElementById('__wpe_opt_overlay')?.remove();"
        "document.getElementById('__wpe_opt_style')?.remove();",
        -1, nullptr, nullptr, nullptr, nullptr, nullptr);

    if (savedOptionMenu) {
        webkit_option_menu_close(savedOptionMenu);
        g_clear_object(&savedOptionMenu);
    }
}

/*
 * Script message handler: called when user clicks an option menu item.
 * JS sends the item index via postMessage().
 */
static void onOptionMenuAction(WebKitUserContentManager*,
    JSCValue* value, gpointer)
{
    if (!savedWebView || !savedOptionMenu)
        return;

    int index = jsc_value_to_int32(value);
    auto* webView = savedWebView;
    auto* menu = savedOptionMenu;

    if (index >= 0 && index < static_cast<int>(webkit_option_menu_get_n_items(menu)))
        webkit_option_menu_activate_item(menu, index);

    /* Dismiss the menu after activation */
    dismissOptionMenu(webView);
}

/*
 * Build JavaScript that creates the HTML option menu (select dropdown).
 * Reads items from WebKitOptionMenu and generates DOM elements.
 * Returns a newly allocated string (caller must g_free).
 */
static gchar* buildOptionMenuJS(WebKitOptionMenu* menu, WebKitRectangle* rect)
{
    guint nItems = webkit_option_menu_get_n_items(menu);
    if (!nItems)
        return nullptr;

    GString* js = g_string_new("(function() {\n");

    /* Remove existing option menu if any */
    g_string_append(js,
        "var old = document.getElementById('__wpe_opt_overlay');\n"
        "if (old) old.remove();\n"
        "var oldStyle = document.getElementById('__wpe_opt_style');\n"
        "if (oldStyle) oldStyle.remove();\n");

    /* Inject CSS */
    g_string_append_printf(js,
        "var style = document.createElement('style');\n"
        "style.id = '__wpe_opt_style';\n"
        "style.textContent = `%s`;\n"
    ...
    g_string_append(js,
        ...
        "overlay.id = '__wpe_opt_overlay';\n"
        "overlay.addEventListener('click', function(e) {\n"
        "  if (e.target === overlay) {\n"
        "    window.webkit.messageHandlers.optionMenuAction.postMessage(-1);\n"
        "  }\n"
        "});\n");
    ...
    g_string_append(js,
        ...
        "menu.id = '__wpe_opt_menu';\n");

    /* Track whether we need a separator before the first non-group item after a group */
    bool needSep = false;

    for (guint i = 0; i < nItems; i++) {
        auto* item = webkit_option_menu_get_item(menu, i);

        if (webkit_option_menu_item_is_group_label(item)) {
            /* Group label (optgroup heading) — non-interactive */
            const char* label = webkit_option_menu_item_get_label(item);
            if (label) {
                g_autofree char* escaped = g_strescape(label, nullptr);
                g_string_append_printf(js,
                    "var grp = document.createElement('div');\n"
                    "grp.className = '__wpe_opt_group';\n"
                    "grp.textContent = \"%s\";\n"
        ...
        g_autofree char* escaped = g_strescape(label, nullptr);
        bool isSelected = webkit_option_menu_item_is_selected(item);
        bool isEnabled = webkit_option_menu_item_is_enabled(item);
        bool isGroupChild = webkit_option_menu_item_is_group_child(item);

        /* Build class list */
        const char* baseClass = "__wpe_opt_item";
        GString* classStr = g_string_new(baseClass);
        if (isSelected)
            g_string_append(classStr, " __wpe_opt_selected");
        if (!isEnabled)
            g_string_append(classStr, " __wpe_opt_disabled");
        if (isGroupChild)
            g_string_append(classStr, " __wpe_opt_group_child");

        g_autofree char* classes = g_string_free(classStr, FALSE);
        ...
        if (isEnabled) {
            g_string_append_printf(js,
                ...
                "    window.webkit.messageHandlers.optionMenuAction.postMessage(%u);\n"
                "  });\n"
        ...
        /* If next item is a group label, add separator before it */
        if (i + 1 < nItems) {
            auto* nextItem = webkit_option_menu_get_item(menu, i + 1);
            if (webkit_option_menu_item_is_group_label(nextItem))
                needSep = true;
        }
    }
...
/*
 * WebKit show-option-menu signal handler.
 * Intercepts the option menu and shows an HTML-based dropdown instead.
 *
 * WPE 2022 API signature: (WebKitWebView, WebKitOptionMenu, WebKitRectangle)
 */
static gboolean onShowOptionMenu(WebKitWebView* webView,
    WebKitOptionMenu* menu, WebKitRectangle* rect, gpointer)
{
    /* Save state for action handler */
    savedWebView = webView;
    g_clear_object(&savedOptionMenu);
    savedOptionMenu = WEBKIT_OPTION_MENU(g_object_ref(menu));

    /* Build and inject the HTML option menu */
    g_autofree char* js = buildOptionMenuJS(menu, rect);
    if (js) {
        webkit_web_view_evaluate_javascript(webView, js, -1,
            nullptr, nullptr, nullptr, nullptr, nullptr);
    }

    /* Return TRUE to indicate the signal is handled */
    return TRUE;
}
 ...
 *
 * WPE 2022 API signature: (WebKitWebView, WebKitContextMenu,
...
static void activate(GApplication* application, gpointer)
{
    ...
    /* 4. User content manager (for script message handlers) */
    ...
    webkit_user_content_manager_register_script_message_handler(
        userContentManager, "optionMenuAction", nullptr);
    g_signal_connect(userContentManager,
        "script-message-received::optionMenuAction",
        G_CALLBACK(onOptionMenuAction), nullptr);
    ...
    g_signal_connect(webView, "show-option-menu",
        G_CALLBACK(onShowOptionMenu), nullptr);
...
int main(int argc, char* argv[])
{
    ...
    /* Point WebKit to the injected bundle from the build tree */
